...
| **`spakky-saga`** | 분산 트랜잭션 Saga 오케스트레이션(SagaFlow, SagaStep, 보상 처리, AuthContextSnapshot 전파) |
...
| **`spakky-cryptography`** | 암복호화, 해시/HMAC, 스냅샷 서명, 패스워드 해싱 제공자 |
...
pip install "spakky[events-rabbitmq]"          # RabbitMQ 이벤트 서비스
pip install "spakky[database-postgres]"        # SQLAlchemy + PostgreSQL driver
```

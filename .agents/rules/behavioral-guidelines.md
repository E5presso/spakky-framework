...
- **후속 티켓 자동 실행 default = 즉시 분해 (이벤트 트리거 기반).** 본 세션 내 백그라운드 서브에이전트로 `/process-ticket {신규-TICKET-ID}` 즉시 실행. 후속이 현재 PR 산출물에 의존하거나 동일 파일 충돌이 예상되면 `blockedBy`를 설정한 뒤에도 `/process-ticket`를 spawn하여 Phase 1.5 blocker polling에 진입시킨다. 실행 주체 없는 차단 경로 보고 금지. Agent/Task 도구 부재로 spawn이 불가능하면 PR reply 금지 후 사용자에게 `executor-unavailable` blocker로 보고한다. **휘발성 스케줄러(`CronCreate` session-only·`ScheduleWakeup` 현 세션 wake-up·로컬 cron·sleep 루프) 절대 금지** — 세션 종료 시 사라져 약속 유기와 동등. 상세는 `/triage-comments` SKILL.md "후속 티켓 생성" 참조.

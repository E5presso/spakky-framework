...
- `Any` 정책은 `type-discipline.md` §5가 SSOT. `Any`·`object`·`cast()`는 1줄 사유 없이 사용하지 않는다.
...
- `src/` 내에서 빌트인 예외 클래스를 직접 `raise` 금지 — 커스텀 에러 정의
...
- `@override` 정책은 `type-discipline.md` §6이 SSOT. 부모 클래스 메서드·ABC abstract method 구현에는 `typing.override`를 붙인다.
...
- silent fallback 금지: `pass`, `return None`, 기본값 반환 금지. 예: `except SomeError: pass` ❌, `except SomeError: raise` ✅
- 로깅: 모듈 레벨 `getLogger(__name__)` (DI 주입 금지)
- Docstring: Google Python Style Guide
- 매직 넘버 금지 — 명명된 상수로 정의. 예외: `0`, `1`, `-1` (인덱싱/비교), 빈 컬렉션 길이 `0`은 인라인 허용

...
> **리뷰 fan-out**: `/review-code`의 카테고리 SSOT는 `.agents/harness/rules/review-heuristics.md` 14개 카테고리.
...
2. **4-2b 구현 에이전트 응답** — 각 의문점에 대해 **수용(코드 수정)** / **반박(근거 제시)** / **후속 이슈 위임**(Warning 한정) 중 택일
   ...
   - 반박: 근거를 작성한다. 반박 근거는 아래 4가지 중 하나 이상을 충족해야 한다:
     1. **규칙 인용** — `.agents/harness/rules/<file>.md`의 구체적 규칙을 인용하고 해당 규칙이 현 상황에 적용됨을 논증

...
- charter §2 3-축 정렬에서 **명백한 충돌** — 이슈 어휘가 `domain.md`·`monorepo.md`·도메인 사전(CLAUDE.md "프로젝트 특수 컨벤션")과 어긋남, 또는 이슈가 가정하는 산출물이 코드베이스에 다른 형태로 이미 존재
...
2. 코드 SSOT (`core/spakky*/src/`, `plugins/spakky-*/src/`) + 도메인 사전(CLAUDE.md / ARCHITECTURE.md) + 관련 ADR(`docs/adr/`)만 참조한다. stale 가능성이 있는 외부 디자인 문서는 참조 금지.
...
4. **사용자 질의 트리거 (이것만)** — 아래에 해당하는 경우만 사용자 질의. **sub-agent로 실행 중이면 SKILL.md "사용자 질의 위임" 절의 `ask-delegate`로 메인에 위임**, 사용자 직접 호출이면 `AskUserQuestion` 직접:
   ...
   - **하네스 교정** — 변경 경로가 `.agents/harness/skills/`·`.agents/harness/rules/`·`CLAUDE.md` (또는 동등 하네스 자산)을 포함하면 plan summary를 `ask-delegate`로 메인에 질의하고 회신 수신 후에만 Phase 5 진입한다. 하네스는 후속 모든 세션의 행동을 결정하는 SSOT이므로 sub-agent의 "low risk·single-file edit" 자가 판정으로 우회할 수 없다. 사용자 직접 호출이면 `AskUserQuestion` 직접.
...
2. 계획 수립 시 준수 사항:
   - 프로젝트 CLAUDE.md의 모든 규칙
   - `.agents/harness/rules/charter.md` (헌장)
   - `.agents/harness/rules/behavioral-guidelines.md`의 행동 원칙
   - `.agents/harness/rules/python-code.md`, `.agents/harness/rules/domain.md`, `.agents/harness/rules/aspect.md`, `.agents/harness/rules/plugin.md`, `.agents/harness/rules/monorepo.md`
   - 레이어 의존 방향 (외부 → 내부만 허용)

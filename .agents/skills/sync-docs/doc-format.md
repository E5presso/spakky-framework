...
- 추가 가이드라인은 `.agents/rules/mermaid.md` 참조 (중첩 서브그래프, 인라인 엣지, 노드 색상).
...
3. Aspect 작성 시 `.agents/rules/aspect.md` 규약 요약 (동기/비동기 쌍 의무, pointcut 표현)
...
**소스**: `core/*/src/`, `plugins/*/src/` 전체에서 실제 패턴 추출 + `.agents/rules/python-code.md`, `.agents/rules/test-writing.md`
...
3. 의존성 관리 규칙 (`.agents/rules/dependencies.md` 요약: PyPI 버전 조회, 내부 의존성 워크스페이스 표기)

...
Phase 1에서 수집한 이슈 본문(`gh issue view 또는 GitHub connector 조회`)에서 **"수용 기준" / "Acceptance Criteria" 섹션의 grep / find / rg / 코드 식별자 검증 라인**을 추출한다. 추출 대상:

- 코드 fence 안의 `grep ...`, `rg ...`, `find ...`, `ls ...` 명령 라인 (기대값 명시: "0 hits", "no match", "exit 1" 등).
- 본문이 명시한 식별자 부재/존재 검증 (예: "`coerce_legacy_*` 함수 0개", "`adapters/services/config_calibrator.py` 파일 없음").
...
- **1건 이상 미충족 (FAIL)**: Phase 5 진입 차단. 미충족 라인 + 실제 출력을 구현 에이전트에게 다시 넘겨 추가 구현 + Phase 4.5 재실행. 동일 이슈에서 본 게이트가 2회 연속 미충족이면 charter §4-A 질의 트리거(스펙↔코드 직접 충돌)로 판단하여 `ask-delegate`로 메인에 위임 (sub-agent) / `사용자 질의` (사용자 직접 호출). 사용자가 "부분 머지 후 후속 처리"를 명시 선택하면 그 시점에 한해 `acceptance_check: partial` + Phase 5 진입이 허용된다 (정상 경로 아님 — 사용자 승인 예외).
- **본문에 grep 라인 없음 (missing)**: `acceptance_check: missing`으로 기록하고 Phase 5 진입. 본 케이스는 본문 자체에 외부 검증 가능한 기준이 없는 것이므로 본 게이트는 차단 책임이 없다. autopilot Track D 외부 검증 routine은 본 케이스에서 PR diff vs 본문 명세 LLM 비교로 보강한다.
...
- [x] `grep -r 'from .*adapters.services.*config_calibrator' /src` → 0 hits (실제: 0)
- [x] `find /src -name 'coerce_legacy_*'` → 0 hits (실제: 0)
- [ ] `grep -r 'evaluator_graph_legacy' /src` → 0 hits (실제: 3) ← 미충족
```

autopilot 메인은 PR 본문에서 본 섹션을 회수하여 Track D 외부 검증의 입력으로 사용한다.

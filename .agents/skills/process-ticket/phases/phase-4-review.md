...
> **Phase 진입 ping** (sub-agent 한정): (a) 구현 agent spawn 직전 1회 SendMessage(to: "team-lead", message: `phase: Phase 4 implement | issue: <id> | impl dispatched`); (b) 매 review iteration 진입 직후 1회 SendMessage(to: "team-lead", message: `phase: Phase 4 review iter <N> | issue: <id> | critical <count>`). SKILL.md "Phase 전환 progress ping" SSOT.
...
> **리뷰 fan-out**: `/review-code`는 단일 서브에이전트가 14개 카테고리 전체를 순회한다. 카테고리 SSOT (Single Source of Truth)는 `personas/*.md` 5개 파일이다.

> **종료 조건 및 모순 에스컬레이션**: Critical만 신규 생성을 차단하고, Warning은 즉시 수정·반박·**후속 이슈 위임** 중 택일 (기술 부채로 전환 허용). Info 심각도는 리뷰어가 생성하지 않는다. 또한 새 iteration이 **직전 iteration에서 해소된 결정을 뒤집는 의문점**을 내면 자동 루프 중단 후 사용자에게 확정 방향 요청 — sub-agent로 실행 중이면 SKILL.md "사용자 질의 위임" 절의 `ask-delegate`로 메인에 위임 (`phase: Phase 4-2`, `trigger: review-loop-flip`), 사용자 직접 호출이면 `사용자 질의`를 직접. 상세는 `/review-code` 스킬 참조.

## 리뷰 격리 채널 (외부 게이트 보존)

`charter.md` §5와 `AGENTS.md` "서브에이전트 워크플로"는 `/review-code`를 **구현자와 분리된 컨텍스트에서 실행되는 외부 게이트**로 규정한다 — self-confirmation bias 회수가 목적이다. 리뷰 에이전트가 구현 에이전트와 같은 컨텍스트를 공유하면 게이트가 무력화된다. 본 스킬의 호출 경로에 따라 `/review-code` 실행 메커니즘이 갈리며, 4-2a는 본 절이 정한 메커니즘으로 리뷰 에이전트를 디스패치한다.

본 스킬이 `Agent` tool을 보유했는지로 경로를 판별한다 (toolset 직접 확인):

1. **`Agent` tool 보유** (사용자 직접 호출 `/process-ticket` 등) — 기존대로 `/review-code`를 독립 서브에이전트(`Agent`, `opus`)로 직접 spawn한다. 격리는 `Agent`가 새 컨텍스트를 여는 것으로 보장된다.
2. **`Agent` tool 부재 + team-lead 라우팅 가능** (autopilot이 §3-2 형식으로 spawn한 teammate 컨텍스트 — `team_name` 발급) — teammate 컨텍스트에는 `Agent` tool이 없어 자체 격리 spawn이 불가능하다. `/review-code`를 in-context로 수행하면 외부 게이트가 무너지므로, SKILL.md "리뷰 위임 (sub-agent 한정)" 절의 `review-delegate` 메시지를 team-lead에 보내 격리 리뷰를 위임하고 `review-result` 회신을 수신한다. 하네스 교정 이슈이면 `/evaluate-harness` 동반을 `review-delegate`에 명시한다.
3. **`Agent` tool 부재 + team-lead 라우팅 불가** (위임 채널 자체가 없는 예외 경로) — 격리 리뷰가 구조적으로 불가능하다. 이때만 in-context `Skill(review-code)`로 수행하되, (a) 리뷰 직전 변경 파일 + 관련 `rules/`·`personas/`를 fresh-slate로 재독하고, (b) PR 본문에 "리뷰가 격리 게이트 없이 in-context로 수행됨 — 외부 게이트 미적용" 한계를 명시 노출하여 사람 리뷰어가 보강 판단할 수 있게 한다.

경로 1·2가 가능하면 경로 3을 선택하지 않는다 — 경로 3은 외부 게이트를 복원하지 못하는 차선 완화이며, 격리 경로가 존재하는데도 in-context로 수행하는 것은 charter §5 위반이다.
...
`/check` (ty + test + 레이어 의존)는 **리뷰 루프에 포함시키지 않고**, 수렴 직후 Phase 5(커밋) 진입 직전에 1회만 실행한다. 이유: 매 iteration마다 ty/test를 돌리면 느리며, 리뷰 루프의 본질(합리적 의문점 수렴)과 무관한 기계적 회귀 검출이다. 타입/테스트 회귀는 최종 게이트에서 한 번에 걸러내도 충분하다.

**한 iteration의 흐름**:

1. **4-2a `/review-code`** — "리뷰 격리 채널" 절이 정한 경로(독립 서브에이전트 / `review-delegate` 위임 / in-context 차선)로 합리적 동료 리뷰 실행
   - 리뷰 에이전트에게 **이슈 맥락**(목표, 수용 기준, 제약 사항)을 함께 전달한다. 위임 경로에서는 이 맥락을 `review-delegate` 메시지에 동봉한다.
...
2. **4-2b 구현 에이전트 응답** — 각 의문점에 대해 **수용(코드 수정)** / **반박(근거 제시)** / **후속 이슈 위임**(Warning 한정) 중 택일
   ...
   - 반박: 근거를 작성한다. 반박 근거는 아래 4가지 중 하나 이상을 충족해야 한다:
     1. **규칙 인용** — `.agents/rules/<file>.md`의 구체적 규칙을 인용하고 해당 규칙이 현 상황에 적용됨을 논증
     2. **코드베이스 선례** — 같은 패턴이 코드베이스에 이미 있음을 증명 (파일:라인 인용)
     3. **스코프 근거** — 현 이슈 스코프를 벗어남을 증명하고, 후속 이슈 생성(`/plan-issues` 호출)을 약속
     4. **도메인 근거** — 도메인 사전/스펙 문서에서 해당 판단의 근거를 인용
...
3. **4-2c 리뷰 에이전트 재검증** — 구현 에이전트의 응답을 평가
   ...
   - 후속 이슈 위임 응답: 생성된 이슈 번호가 있으면 **인정(폐기)**. 이슈이 없으면 **기각**.
...
2. **최종 `/check`**: 수렴 직후 1회 실행. ty + test + 레이어 의존 모두 통과해야 Phase 5 진입.
...
- **Default 모드**: 사용자 escalation. 남은 미해소 의문점과 직전 반박 기각 내용을 동봉하여 "(1) 수동 수정 후 재개 / (2) 반박 근거 추가 후 재검증 / (3) 현 상태로 PR 생성 / (4) 작업 중단" 중 선택받는다 — sub-agent로 실행 중이면 SKILL.md "사용자 질의 위임" 절의 `ask-delegate`로 메인에 위임 (`phase: Phase 4-3`, `trigger: max-iteration-exceeded`), 사용자 직접 호출이면 `사용자 질의`를 직접.

...
> **절대 명령 (서브에이전트 본인 책무).** `monitor-pr` SKILL.md가 Phase 6 monitor의 SSOT다. 호출자는 자기 turn 안에서 30초 고정 cadence의 `watch.sh`를 포그라운드 Bash로 호출하고, EVENT는 같은 turn에서 case 분기 후 INIT으로 복귀한다. DONE(`merged`, `mergeable-clean`, `closed-without-merge`, `awaiting-human-review`)이 나올 때까지 turn 종료 금지. `Monitor`, `run_in_background`, `ScheduleWakeup`, `CronCreate`, 추가 sleep/backoff, 별도 `gh pr view/checks`로 우회하지 않는다.
>
...
> **MUST**: GitHub Actions / 모든 required check가 pass하고 review bot HEAD 평가가 끝날 때까지 Phase 7로 넘어가지 않는다. `BLOCKED` 상태에서 Phase 7로 전환하면 병합이 불가능하다. 자동화 봇 리뷰어가 pending이면 MERGEABLE 판정 보류.

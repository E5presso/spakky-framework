...
`--auto-merge` 모드와 autopilot 하위 실행을 제외하면, 본 스킬의 산출물은 **PR이라는 폼팩터를 통해 사용자와 의사소통**하는 매체다. 코드 변경은 PR 본문·diff·코멘트·리뷰 스레드로 표현되고, 사용자의 결정 질의·반응·재지시도 같은 폼팩터 안에서 이루어진다.
...
- **사용자 질의 트리거 (이것만)**: 머지 실행(단, `--auto-merge` 또는 autopilot 하위 실행 제외), charter §1 명백한 어긋남(이슈 어휘 ↔ 코드/사전 충돌), 도메인 사전 미등록 신규 어휘 도입, 이슈 분해 단위 재정의 필요, 정책·규칙 위반 가능성, 외부 시스템 destructive action.
- **금지된 질의 패턴**:
...
### autopilot 하위 실행의 암묵적 auto-merge

본 스킬이 `/autopilot`의 wave/resume/meta-fix sub-agent로 호출된 경우, `--auto-merge`가 명시되지 않았더라도 **암묵적으로 `--auto-merge` 모드로 간주**한다. Autopilot 호출 자체가 해당 티켓 집합의 clean PR squash merge 사전 승인이다.

- Phase 7에서 `AskUserQuestion` 또는 `ask-delegate`로 병합 승인을 요청하지 않는다.
- `.process-state.json`에 `phase7_ready`가 있고 `merged`가 없으면, 추가 monitor 없이 Phase 8 squash merge + cleanup을 즉시 수행한다.
- sub-agent가 `status: blocked`, `merge approval required`, `awaiting merge approval` 같은 반환을 내면 호출자(autopilot)는 형식 위반/merge-gate-stuck으로 보고 `--auto-merge` resume을 수행한다.
...
리뷰 루프 수렴 직후 + Phase 5(commit/push/PR) 진입 직전에 **이슈 본문의 "수용 기준" 섹션에 명시된 grep 라인을 워크트리에서 실제 실행**하여 모든 라인의 기대값 일치 여부를 검증한다. 미충족 1건 이상이면 Phase 5 진입 차단 — 서브에이전트는 미충족 항목을 추가 구현 후 본 게이트 재실행. 본 게이트는 `/review-code`가 잡지 못하는 결함(본문 흡수 요청 중 일부만 처리·삭제 대상 import 잔존 등)을 수용 기준 grep 라인의 기계적 실행으로 차단한다.

**상세**: `phases/phase-4_5-acceptance-grep.md`

## Phase 4.6: 문서 동기화 게이트

코드 변경이 public API, 환경변수, 패키지 README, 사용자 가이드/API 문서, `ARCHITECTURE.md`, `CONTRIBUTING.md` 중 하나에 영향을 주면 Phase 5 전에 `/sync-docs`를 실행한다. 영향이 없으면 `notes: docs-sync:not-needed`를 반환 또는 PR 본문에 남긴다. Autopilot Phase 5의 `/sync-docs`는 최종 sweep이며, 단일 `/process-ticket`의 문서 동기화를 대체하지 않는다.

## Phase 5: 커밋 & PR 생성

`/sync-docs` 필요 여부 확인 → `/commit` → `git push -u origin HEAD` → `/create-pr {ISSUE-NUMBER}` → `/update-project-status {ISSUE-NUMBER} "In Review"` 명시적 호출. 자동 진행.
...
PR이 merge 가능 상태가 되면 기본 직접 호출에서는 `AskUserQuestion`으로 병합 승인 요청. `--auto-merge` 또는 autopilot 하위 실행은 즉시 Phase 8로 진행한다. `--overnight` 모드는 상태 요약 후 즉시 반환(Phase 8 생략).
...
| Phase 6 `DONE reason=mergeable-clean` 직후 | `phase7_ready` | `{ "reason": "mergeable-clean", "pr": N, "mergeState": "CLEAN|UNSTABLE", "pendingChecks": 0, "failedChecks": 0 }` |
...
- `root_guard`: Phase 3에서 `assert_worktree_isolation.sh --init`가 기록한 루트 리포지토리 기준선 (`repo_root`, `root_branch`, `root_head`, `root_status_hash`, `worktree`, `branch`). Phase 4~5 guard는 이 기준선과 현재 상태를 비교하여 root checkout mutation을 차단한다.
...
- `phase7_ready` 존재 + `merged` 부재 → autopilot 하위 실행이면 Phase 8 squash merge + cleanup부터 재개한다. monitor가 이미 `mergeable-clean` terminal DONE을 반환했으므로 추가 watch/poll이나 병합 승인 질의를 돌리지 않는다.
- `pr_opened` 존재 + `merged` 부재 → autopilot은 resume sub-agent를 `--auto-merge`로 띄워 monitor/머지/cleanup을 이어받게 한다 (charter §5 "메인 블로커 회피 금지").
...
각 phase 본문(`phases/phase-3-worktree.md`, `phase-5-commit.md`, `phase-8-merge-cleanup.md`)에 호출 코드가 명시되어 있다 — "암묵적으로 알아서 한다" 금지.
...
예외: autopilot 하위 실행의 Phase 7 병합 승인은 위임 대상이 아니다. Autopilot 호출 자체가 clean PR merge 사전 승인이라서 sub-agent는 `ask-delegate`를 보내지 않고 Phase 8로 진행한다.
...
단, `phase: Phase 7` + `trigger: merge-approval`이 autopilot 하위 실행에서 도착하면 메인은 사용자에게 묻지 않고 `choice: recommended-auto-applied`로 회신한다. 이 메시지 자체가 merge-gate-stuck 시그널이므로 autopilot ledger에도 기록한다.
...
| Phase 7 | 정규 모드 머지 승인 (`--auto-merge`/`--overnight` 미지정, autopilot 하위 실행 아님) | sub-agent 한정 위임 |
...
pending_human_comments: URL,...|none
spawned: #N,...|none
...
- `pending_human_comments`는 `status: awaiting-review`일 때 사람 응답이 필요한 bot CH2 코멘트 URL 목록. 그 외 status는 `none`.
...
^acceptance_check: (PASS|partial|missing)$
^issue_status: (Done|InReview|InProgress|Backlog|Cancelled)$
^pending_human_comments: (https?://[^, ]+(,https?://[^, ]+)*|none)$
^spawned: (#\d+(,#\d+)*|none)$
```

조건부 검사: `status: failed`이면 `^failed_reason: .+$`가 반드시 1줄 존재해야 하며, `status != failed`이면 `failed_reason` 라인을 포함하지 않는다.
...
내부 sub-agent spawn(Phase 2-2 / 4-1 / 4-2 / 4-5 등)은 메인이 bypass permissions 모드일 때만 `permission_mode: "bypassPermissions"`를 명시하고, 일반 모드에서는 키를 생략한다. autopilot 하위 실행은 inbound spawn 정책을 이미 받지만, 본 스킬 내부 spawn은 메인 세션 모드를 다시 1회 평가해 같은 분기를 적용한다.
...
| 단계 | 모델 |
|------|------|
| Phase 1 이슈 분석 / 컨텍스트 수집 | `sonnet` |
| Phase 2 구현 계획 수립 (Plan agent) | `opus` |
| Phase 3 워크트리 생성 (`/create-worktree`) | `haiku` |
| Phase 4 구현 루프 (코드 작성) | `opus` |
| Phase 4 검증 (`/check`) | `sonnet` |
| Phase 4 리뷰 (`/review-code`) | `opus` |
| Phase 5 커밋 메시지 / PR 본문 생성 | `sonnet` |
| Phase 5 `git push` · `gh pr create` 실행 | `haiku` |
| Phase 6 `/monitor-pr` polling/상태 분기 | `haiku` |
| Phase 6 `/triage-comments` 판단 | `opus` |
| Phase 7 머지 게이트 상태 제시 | `haiku` |
...
- **PR 메타 mutation도 정의된 절차만 허용한다.** 코멘트 reply는 `/triage-comments`, PR 생성 메타는 `/create-pr`, 재리뷰 요청은 `/monitor-pr`의 명시 절차에서만 수행된다.
- **스크립트 출력이 부족해 보이면 스크립트를 수정한다.** 메인이 임의 명령으로 보강하지 않는다 — `/optimize-harness`로 진입.

## 규칙

- **사용자 확인 구간은 모드별로 다르다.** 기본 직접 호출: Phase 7 (PR 병합) 1곳. `--require-approval`: Phase 2 + Phase 7 두 곳. `--overnight`: 없음 (Phase 2 생략, Phase 7은 병합하지 않고 상태만 반환). `--auto-merge`: 없음 (Phase 7 승인 생략, mergeable 즉시 병합). autopilot 하위 실행: 없음 (암묵적 `--auto-merge`). 그 외 단계는 전부 자동 진행.
...
- 단, autopilot 하위 실행에서는 process-ticket sub-agent가 곧 Phase 6 실행 주체다. team-lead에 polling을 위임하거나 `phase7_ready`만 반환하지 않고, `--auto-merge` 의미를 이어받아 같은 turn 안에서 Phase 8까지 진행한다.
- Phase 6에서 리뷰 코멘트 처리 완료 후 해당 리뷰어에게 재리뷰를 요청한다.
- Phase 7에서 사용자 승인 없이 병합하지 않는다. `--overnight` 모드에서는 Phase 7이 **병합을 수행하지 않고 상태만 반환**하므로 이 규칙은 자동으로 지켜진다. `--auto-merge` 모드와 autopilot 하위 실행에서는 사용자가 호출 시점에 사전 승인한 것으로 간주하여 mergeable 상태가 되면 즉시 병합한다.
...
- **`apply_patch` 사전 차단**: 격리 워크트리 수정에서 `apply_patch`는 세션 CWD가 대상 워크트리 루트임을 pre-edit guard로 확인한 경우에만 사용한다. 도구 호출별 `workdir`만 지정 가능한 컨텍스트에서는 별도 `apply_patch` 도구를 금지하고 `safe_worktree_apply_patch.sh` wrapper를 워크트리 cwd에서 실행한다 (Phase 4 "`apply_patch` 라우팅 규칙" 참조).

...
1. **부모 epic 의존 재작성 (Phase 1 epic 제외와 짝)**: Phase 1 §3에서 부모 epic을 대상 집합에서 제외했으므로, 그 epic을 `blockedBy`로 가진 자식 이슈의 의존은 단순 누락된다 — 이대로 DAG를 짜면 그 자식이 epic의 자식 이슈 묶음 머지를 기다리지 않고 조기 진입한다. 따라서 DAG 구성 전, 각 이슈 `T`의 `blockedBy`를 epic 전개로 재작성한다.
   ...
   **알고리즘**:
   ...
   5. epic 식별 기준은 Phase 1 §3과 동일 (자식 이슈 보유 + 자식 중 하나 이상이 대상 집합에 속함). 식별 결과를 Phase 1과 Phase 2가 공유한다.

2. **집합 내부 의존만 반영**:
   - §1 재작성 후 각 이슈의 `blockedBy`에서 **대상 집합에 속한** 이슈만 엣지로 추가.
...
5. **사용자 보고 (승인 게이트 없음)**:
   - 웨이브별 이슈 수 요약 테이블.
   - Mermaid `flowchart LR` 다이어그램(노드=이슈, 엣지=blockedBy).
   - 보고 직후 Phase 3 자동 진입.

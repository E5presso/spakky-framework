...
"caller 없음 → YAGNI 반론"으로 즉시 흐르지 않는다. `.agents/rules/review-heuristics.md` 원칙 5(YAGNI 역설)에 따라 다음을 모두 확인한 뒤 판정:
...
스코프를 벗어난 작업을 분리한다고 답했다면, PR 스레드 응답 전 **티켓 생성·의존 설정·처리 경로**까지 확정한다. "후속 티켓을 만들겠다" 또는 티켓 URL만 남기면 유기다.

1. `plan-issues` 또는 `gh issue create`로 티켓 생성.
2. `.agents/rules/behavioral-guidelines.md`의 `blockedBy` 규칙대로 의존 관계 설정. 현재 티켓을 자동 blocker로 삼지 말고, 공유 인프라를 정의한 선행 티켓이 실제로 있을 때만 연결한다.
3. `gh issue view {N}` 또는 Sub-issues GraphQL read-back으로 의존 관계 확인.
4. 본 세션 내 백그라운드 서브에이전트로 `process-ticket {신규-이슈번호}`를 즉시 실행한다. 현재 PR 산출물 의존·동일 파일 충돌이면 `blockedBy` 설정 후에도 agent를 spawn하여 Phase 1.5 blocker polling으로 대기시킨다. 실행 주체 없는 차단 경로는 금지. Agent/Task 도구 부재로 spawn이 불가능하면 PR reply 금지 후 사용자에게 `executor-unavailable` blocker로 보고한다. `CronCreate`·`ScheduleWakeup`·로컬 cron·sleep 루프 같은 휘발성 스케줄러는 금지.
5. PR 스레드 reply에는 (a) 티켓 URL, (b) blocker 목록, (c) 처리 경로(`process-ticket` agent 실행) 세 가지를 모두 포함한다. 하나라도 없으면 reply 금지. `executor-unavailable`이면 PR reply를 남기지 않고 사용자 보고로 종료한다.

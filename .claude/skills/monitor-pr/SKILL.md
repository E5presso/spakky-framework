...
`watch.sh`가 `DONE reason in {merged, approved-clean, closed-without-merge, awaiting-human-review}`을 emit하는 순간 본 스킬은 종료다. 호출자는 그 시점에 **추가 `watch.sh` / `poll.sh` / `gh pr view` 호출을 만들지 않는다.** terminal reason 처리는 reason별로 1:1 고정이며 추가 cycle을 요구하지 않는다:

- `reason=merged` → Phase 8 cleanup 1회 실행 후 turn 종료.
- `reason=approved-clean` → Phase 7 머지 게이트로 전환 후 turn 종료.
...
- **임의 shell 명령 호출 절대 금지.** 호출자는 본 스킬이 명시한 스크립트(`poll.sh`, `collect_comments.sh`)와 명시된 다른 스킬(`/review-pr`, `/plan-issues`, `/check`, `/commit`)만 호출한다. 아래는 모두 금지:
...
- **PR/이슈 mutation도 정의된 sub-skill만.** PR 메타 변경(`gh pr edit`), 코멘트 reply, 라벨 추가 등은 모두 `/review-pr` 등 정의된 sub-skill을 통한다. 호출자가 직접 `gh pr edit` / `gh api ... replies` 호출 금지. (코멘트 reply 마커 규칙은 sub-skill이 책임진다.)
...
LISTENING (watch.sh 단일 포그라운드 호출 — EVENT 또는 DONE까지 블록)
  ...
  └─ DONE  reason=approved-clean | merged  → 종료 (Phase 7)
```
...
`TOTAL > 0` → `/review-pr {PR}` 무조건 실행. 예외 없음.
`TOTAL == 0` → polling 시작.

**금지**: 에이전트가 코멘트 내용을 보고 "처리 불필요"로 판단하고 triage를 건너뛰는 것. "무지성 반영"(코멘트를 보고 곧바로 코드를 수정하는 것)도 동일하게 금지 — 판단은 `/review-pr`에서만 이루어진다.
...
failedChecks=<N>
reason=<comments-changed|review-decision-changed|ci-failed|merge-dirty|approved-clean|merged|awaiting-human-review>
...
case "$MARKER:$REASON" in
  "EVENT:comments-changed")        # STALE_HANDLED_IDS=$STALE collect_comments.sh → /review-pr → INIT
    ;;
  "EVENT:review-decision-changed") # collect_comments.sh → /review-pr → INIT
    ;;
  ...
  "EVENT:heartbeat")                # SendMessage tick ping 1줄 → INIT (watch.sh 재호출, cycle 카운터 0부터 재시작)
    ;;
  "DONE:approved-clean"|"DONE:merged")
    # Phase 7/8 전환
    ;;
...
| `DONE reason=approved-clean` | 종료 (Phase 7 전환) |
...
수집된 TOTAL > 0이면 반드시 `/review-pr {PR}`을 호출한다. `STALE_HANDLED_IDS`에 포함된 id는 기존 reply 마커가 있어도 미처리로 재분류되어 본문이 재수집된다 — claude bot이 in-place로 갱신한 review/코멘트의 변경 본문이 triage에 다시 노출된다.
...
5. `/review-pr`이 변경된 본문에 대해 재triage 수행.
...
> **PR close/reopen 금지** (CLAUDE.md 절대 금지 사항). 빈 커밋 push만 허용.
...
이 상태는 rebase 후 동일 트리·force-push 후 사실상 hash 미변경 등으로 봇이 신규 커밋으로 인식하지 않아 재리뷰 트리거가 누락된 정체다. CI는 끝났고 리뷰 승인은 들어오지 않으므로 polling만 무한 반복된다.
...
`mergeState in (CLEAN, UNSTABLE)` + `reviewDecision=APPROVED` + `failedChecks=0` → Phase 7 전환.

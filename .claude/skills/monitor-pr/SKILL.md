...
| 병합 차단 | `mergeStateStatus == BLOCKED` (정책상 병합 불가: 미해결 스레드, 승인 부족, CODEOWNERS 등) | `BLOCKED` |
...
| 4 | `OPEN_COMMENT` / `OPEN_REVIEW` / `UNRESOLVED_THREAD` | 코멘트 수집 → `/review-pr` → 반영 + 스레드 resolve → 재시작 |
| 5 | `BLOCKED` | 원인 분류: (a) 미해결 review thread → `/review-pr`로 응답 + resolve → 재시작, (b) 승인 부족 / CODEOWNERS 미충족 → 사용자 개입 요청 (Phase 7 전환) |
| 6 | `MERGEABLE` | Phase 7 전환 |

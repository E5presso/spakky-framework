...
코드 변경 사항을 감지하여 프레임워크 **사용자** 대상 문서를 **코드 기반으로** 동기화한다. import 경로, 클래스 시그니처, 코드 예시, 용어 정의 등이 실제 코드와 일치하는지 검증하고 수정한다.
...
- 가이드 신규 생성은 하지 않는다 — 기존 가이드의 정확성만 보장
...
3. 새로 추가된 핵심 개념이 누락되지 않았는가 (데코레이터, 스테레오타입, 스코프 등)
...
문서의 코드 예시에 등장하는 모든 클래스·함수 호출에 대해:

- [ ] 생성자 파라미터 이름과 순서가 실제 `__init__` 시그니처와 일치하는가?
- [ ] 메서드명이 실제로 존재하는가? (오타, 이름 변경 누락)
- [ ] 메서드의 파라미터 이름·타입 힌트·기본값이 실제 코드와 일치하는가?
- [ ] 반환 타입이 문서 설명과 일치하는가?

### 3-3. 데코레이터 검증

- [ ] 데코레이터 이름이 실제로 존재하는가? (`@Pod`, `@Aspect`, `@EventHandler` 등)
- [ ] 데코레이터 파라미터가 실제 시그니처와 일치하는가? (이름, 타입, 기본값)
- [ ] 데코레이터 import 경로가 유효한가?

### 3-4. 코드 예시 정합성 검증

각 코드 블록에 대해:

- [ ] 예시가 사용하는 모든 클래스·함수가 import되어 있는가?
- [ ] 타입 힌트가 실제 코드의 타입과 일치하는가?
- [ ] 예시의 호출 패턴이 현재 API와 호환되는가? (비동기/동기 혼용 없는가?)
- [ ] 상속 관계가 정확한가? (예: `AbstractAggregateRoot[UUID]`의 타입 파라미터)

### 3-5. mkdocstrings 디렉티브 검증

- [ ] 모든 `:::` 디렉티브의 모듈 경로가 실제 Python 모듈로 해석 가능한가?
- [ ] 삭제된 모듈의 디렉티브가 남아있지 않은가?
- [ ] 새로 추가된 공개 모듈의 디렉티브가 누락되지 않았는가?

### 3-6. 내부 링크 검증

- [ ] 파일 참조: `[text](path/to/file.md)` → 파일이 존재하는가?
- [ ] 앵커 참조: `[text](#section)` → 대상 문서에 해당 섹션이 존재하는가?
- [ ] API 참조: `[text](api/core/spakky.md)` → 파일이 존재하는가?
- [ ] 교차 참조: 가이드 ↔ API 문서 ↔ 용어 사전 간 링크가 양방향 유효한가?

### 3-7. 용어·개념 정합성 검증

- [ ] 용어 사전의 각 용어 정의가 코드의 실제 동작과 일치하는가?
- [ ] 코드에서 제거된 개념이 문서에 남아있지 않은가?
- [ ] 에러 계층 구조 문서가 실제 에러 클래스 상속 관계와 일치하는가?

### 3-8. mkdocs.yml 정합성 검증

- [ ] `nav` 섹션의 파일 경로가 모두 존재하는가?
...
- [docs/guides/fastapi.md:87] 예시의 변수명이 관례와 다름 (동작에는 무관)
...
- import 경로: {N}개 검증 완료, {M}개 수정
- 시그니처: {N}개 검증 완료, {M}개 수정
- 데코레이터: {N}개 검증 완료, {M}개 수정
- 코드 예시: {N}개 검증 완료, {M}개 수정
- mkdocstrings 디렉티브: {N}개 검증 완료, {M}개 수정
- 내부 링크: {N}개 검증 완료, {M}개 수정
- 용어·개념: {N}개 검증 완료, {M}개 수정
- mkdocs.yml: 검증 완료

### 변경 없음 (해당 시)

모든 체크리스트를 순회하였으며, 동기화가 필요한 불일치가 없습니다.
```
...
- 변경이 없는 문서는 건드리지 않는다.
- 가이드 문서를 새로 생성하지 않는다 — 기존 문서의 정확성만 보장한다.

...
```
반복:
  1. 서브에이전트 결과에서 수정된 파일 목록 수집
  2. 수정된 파일이 다른 문서에 영향을 주는지 확인
     - 예: sync-dev-docs가 ARCHITECTURE.md 패키지 테이블 수정 → sync-user-docs의 index.md 패키지 테이블과 불일치?
     - 예: sync-user-docs가 가이드를 신규 생성 → mkdocs.yml nav에 반영되었는가?
  3. 불일치 발견 시 해당 서브 스킬을 **수정된 파일만 대상으로** 재실행
  4. 불일치 0건이면 루프 종료

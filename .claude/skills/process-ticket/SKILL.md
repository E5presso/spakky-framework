...
## Phase 4.5: 수용 기준 자가 검증 게이트 (acceptance-grep)

> **외부 게이트**: Phase 4 리뷰가 못 잡은 결함을 차단한다. Phase 5 커밋 직전 의무.

리뷰 루프가 수렴해도, 이슈 본문의 "수용 기준" 항목이 실제로 충족되었는지를 **기계적으로 재검증**한다. 자기확증 편향 차단 + 기준 누락 차단 목적.

### 4.5-1. 수용 기준 추출

이슈 본문에서 "수용 기준" / "Acceptance Criteria" / "Done when" 섹션을 추출한다. 각 항목을 다음으로 분류:

| 유형 | 검증 방법 |
|------|---------|
| **파일/심볼 존재** | `rg -l 'pattern' <path>` 또는 `find` |
| **파일/심볼 부재** | `rg 'pattern'`이 0건 반환해야 함 (제거·삭제 기준) |
| **테스트 통과** | `/check`로 이미 검증됨 — 통과 기록 재확인 |
| **공개 API 시그니처** | `rg 'def name'`로 시그니처 라인 확인 |
| **의존 방향** | `rg 'from <forbidden_pkg>'`로 위반 0건 확인 |
| **본문 변경 (자연어)** | 추출 불가 — 수동 확인 노트만 |

### 4.5-2. 검증 실행

추출된 각 grep 검증을 워크트리에서 실행한다. 결과를 표로 정리:

```
| 기준 | 검증 명령 | 기대 | 실제 | 결과 |
|------|---------|------|------|------|
| `XxxPort` 인터페이스 정의 | `rg -l 'class XxxPort' core/.../src` | 1+ | 1 | ✓ |
| 레거시 `OldYyy` 제거 | `rg 'class OldYyy' .` | 0 | 0 | ✓ |
| ...
```
...
- **모두 ✓**: Phase 5 진입 허용.
- **1개 이상 ✗**: Phase 5 차단. Phase 4 (구현 & 검증 루프)로 복귀하여 미충족 항목 해소. 해소 후 4.5 재실행.
- **본문 변경 (자연어) 항목만 남음**: 1줄 보고로 사용자에게 확인 후 진행.

### 4.5-4. 자동 grep 추출 불가 시

수용 기준이 자연어 위주여서 grep 추출이 어려우면, 추출 시도 결과(0건)와 함께 사용자에게 보고하고 진행한다. 게이트 자체를 건너뛰지 않는다 — **시도와 보고는 의무**다.

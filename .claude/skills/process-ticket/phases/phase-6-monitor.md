...
코멘트가 1건이라도 감지되면 **반드시 `/triage-comments`**을 거쳐야 한다. 메인 에이전트/구현 에이전트가 코멘트를 보고 직접 코드를 수정하는 "무지성 반영"은 금지한다. 판단(수용/반론/보류)은 triage-comments 스킬에서만 이루어지며, 수용으로 판단된 항목만 코드 수정에 반영한다.

...
메인 세션이 어느 모드인지 직접 introspect하는 API는 없으므로, autopilot 시작 시 메인이 1회 자기 모드를 평가하여 본 wave spawn부터 모든 spawn에 동일한 명시 정책을 적용한다. 평가 방법: autopilot 호출자(사용자)가 bypass 모드로 세션을 시작했음이 명시(예: 호출 인자 `--bypass`·CLAUDE.md 합의·세션 시작 메시지의 `bypass permissions` 선언) 또는 첫 sub-agent 도구 호출이 `permission_request`를 트리거하지 않는지 관찰. 모호하면 명시하여 진행 — `permission_mode` 키가 무시되는 환경에서도 호출은 실패하지 않으며, 명시 누락이 sub-agent hang을 야기하는 비대칭이 더 비싸다.
...
미준수 시 §3-6 fallback으로 진입한다 (재실행 또는 워크트리 체크포인트 회수).
...
**SSOT**: `behavioral-guidelines.md` "스펙 검증 / 후속 티켓" — `후속 티켓 자동 실행 default = 즉시 분해 (이벤트 트리거 기반). 본 세션 내 백그라운드 서브에이전트로 /process-ticket {신규-TICKET-NUMBER} 즉시 실행`. 본 §3-3-quinque은 그 SSOT를 autopilot 본문에 박는 절차 — wave 종료까지 spawn을 미루는 default는 본 SSOT 위반.
...
- **누적 진입점**: 본 §3-3-ter 1곳. `monitor-pr` LISTENING 루프·`review-pr`·다른 phase에서 누적하지 않는다 — 중복 카운트 방지(FR-002 정합).
- **스키마** (배열의 각 entry):

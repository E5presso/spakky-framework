...
- `RunAgentInput`: inbound run 계약. 기존 positional 순서를 유지하는 `state_id`, `instruction`, `conversation_id`, `parent_run_id`, `resume`, `message_history`, `model_selection`, `context`, `metadata` 뒤에 `attachments: tuple[ImagePart | AudioPart | VideoPart | DocumentPart, ...] = ()`를 append한다. `effective_conversation_id`는 `conversation_id or state_id`이고, `user_message()`는 attachment가 없으면 기존 plain-string `ModelMessage.user(instruction)`을, 있으면 `TextPart(instruction)` 뒤에 attachment를 순서대로 둔 user message를 만든다. Runner는 `ModelRequest.metadata`를 `{"state_id": run_input.state_id, **run_input.metadata}` 순서로 구성하므로 현재 같은 이름의 caller metadata가 자동 `state_id`를 덮어쓴다. Approval decision은 이 계약이 아닌 signal repository를 통해 전달된다. Model selection은 run-scoped이며 transcript `ConversationTurn`에 저장·상속되지 않는다. 같은 conversation의 다음 turn이나 approval resume에서 selection을 생략하면 model adapter의 default가 다시 적용된다.
- `TextPart`, `ImagePart`, `AudioPart`, `VideoPart`, `DocumentPart`, `MediaSafetyLimits`: ordered multimodal model-input contract. Media는 URI 또는 immutable bytes 중 정확히 하나를 갖고 MIME family와 optional source/digest, document filename을 보존한다. Default는 HTTPS URI, request 전체 media 16개, inline bytes 합계 20 MiB이고 custom limits는 더 작은 aggregate bound로 합성된다. `from_uri()`는 DNS·fetch 없는 side-effect-free value construction이며 network resolution authority는 model adapter에 남는다. Common `ModelResponse`/stream output은 계속 text·structured value·tool call이고 generated-media output contract를 추가하지 않는다.
...
- 내장 전략 4종:
    ...
    - `TrimToolResultsCompactionStrategy(max_characters)`: text `TOOL` role content만 자르고 `call_id`/`tool_name` metadata와 assistant envelope를 보존한다. Multipart TOOL content를 text로 손실 변환하지 않고 거부한다.
    - `SummarizeOldTurnsCompactionStrategy(model, keep_recent, summary_instruction)`: group 경계보다 오래된 history만 보조 모델로 요약하고 최신 assistant/TOOL group 전체를 남긴다. Multimodal history는 text segment와 bounded attachment-count marker만 summary prompt에 넣고 media bytes나 URI를 복제하지 않는다.
...
- `ITaskStore`, `ConversationTurn`: 멀티턴 대화 이력을 `conversation_id`로 영속하는 server-side session 계약(ADR-0013 §6). `ConversationTurn.content`는 계속 text `str`이고 **user/assistant transcript만** 담으며 semantic/episodic/user memory, media attachment, TTL, correction, delete를 소유하지 않는다. 같은 durable run의 media는 runner checkpoint가 복원하지만 다음 server-session turn에 `ITaskStore`가 replay하는 것은 original instruction text와 validated assistant text, 또는 text가 없는 typed final의 deterministic JSON serialization이다. `load_history(conversation_id)`로 이전 transcript를 조회하고 `append_turns(conversation_id, turns)`로 turn을 누적한다. `conversation_id`는 AG-UI `threadId`와 A2A `contextId`로 투영될 수 있는 프로토콜 중립 키입니다. 단, A2A protocol `Task` snapshot 영속은 `spakky-a2a`의 `IA2ATaskRepository`/`SpakkyA2ATaskStore` 책임이고 core `ITaskStore`와 분리됩니다. `AgentRunner`는 `message_history`가 주입되면 그 이력을 우선해 시드하고(stateless, store 미기록), 아니면 store의 영속 이력을 시드한 뒤 완료 시 turn을 기록한다 — 두 경로는 run마다 상호 배타적이다.
...
- `ModelMessage`, `ModelRequest`, `ModelResponse`, `ModelStreamEvent`, `ModelUsage`: provider-neutral model 호출/응답/stream과 usage 계약. `ModelMessage.content`는 기존 `str` shortcut 또는 ordered content-part sequence이고 `ModelMessage.user()`가 canonical user constructor다. `ModelRequest.__post_init__()`은 outer messages/context sequence를 tuple로 고정하고 request metadata를 snapshot하며 nested value type과 request-wide media count/inline-byte aggregate를 검증한다. Nested message/context/manifest/digest/tool/schema deep copy는 `ModelRequest.snapshot()` 책임이고 `spakky-llm` adapter가 complete/stream 진입에서 이를 호출해 cache/provider/store에 하나의 snapshot을 사용한다. `ModelUsage`는 input/output/total에 optional cached input, aggregate cache-write, 5-minute cache-write, 1-hour cache-write token을 보존한다. Runner는 같은 `ModelSelection` 객체를 `RunAgentInput.model_selection`에서 `ModelRequest.model_selection`과 `capability_for(selection)`으로 전달해 실행 route와 capability 조회를 결속한다. Selector를 request metadata로 복제하지 않는다.
...
가장 작은 inbound DX는 기존 instruction에 media attachment tuple을 더하는 방식입니다. `attachments`는 기존 `RunAgentInput` positional field 뒤에 추가됐으므로 기존 positional caller의 순서는 바뀌지 않습니다.

```python
from spakky.agent import ImagePart, RunAgentInput

run_input = RunAgentInput(
    state_id="run-vision-1",
    instruction="이 화면의 오류를 설명하세요.",
    attachments=(
        ImagePart.from_uri(
            "https://assets.example.test/error.png",
            media_type="image/png",
            source="support-upload:42",
        ),
    ),
)
```

Lower-level model caller는 같은 part를 `ModelMessage.user((TextPart(...), ImagePart(...)))`로 순서화할 수 있습니다. 기존 text-only caller는 계속 `ModelMessage.user("...")` 또는 plain `instruction`을 사용합니다. Media type은 image/audio/video/document이고 inline bytes는 `from_bytes()`, remote reference는 `from_uri()`를 사용합니다. URI constructor는 네트워크에 접근하지 않으며 default 16-part/20-MiB aggregate와 URI syntax·scheme·host/literal-address guard만 적용합니다. Application이 private media gateway나 custom scheme을 소유하면 per-part `MediaSafetyLimits(allowed_uri_hosts=...)` 또는 `allowed_uri_schemes`로 권한을 명시하고, 실제 DNS·provider 접근 검증은 adapter port가 수행합니다.

`ModelResponse.content`와 public final output에는 공통 generated-image/audio/video payload를 추가하지 않습니다. Provider가 입력 modality를 지원하려면 selected `ModelCapability.input_modalities`에 정확히 선언되어야 하며, 그렇지 않으면 model 호출 전에 실패합니다.
...
각 save는 positive monotonic `checkpoint_revision`을 올리고 raw checkpoint의 SHA-256 fingerprint, JSON shape size, history length와 model step을 `AgentEvidenceKind.CHECKPOINT`로 append합니다. Resume은 repository 반환 순서가 아니라 가장 큰 revision을 latest로 정하고, state checkpoint revision·raw shape·fingerprint·history length·step이 그 evidence와 정확히 맞는지 **multipart media decode와 remote URI policy 실행 전에** 검증합니다. 따라서 이전 valid checkpoint replay, content/shape 변조와 out-of-order evidence는 `agent_checkpoint_invalid`로 닫힙니다. Initial history fingerprint는 serialized media bytes·URI·per-part limits까지 포함해 MODEL evidence와 다시 대조하고, inline media는 이 raw 검증을 통과한 뒤에만 checkpoint에서 복원합니다. Checkpoint는 original `conversation_id`, `user_turn`, `persist_session_turn`도 bind하고 resume caller의 effective conversation이 달라지면 거부합니다. Resume용 instruction은 original user turn을 덮어쓰지 않으므로 실패 뒤 성공한 server-session run은 `TaskStore`에 `"resume"` marker가 아니라 처음 질문을 기록합니다.

Provider 호출은 성공했지만 downstream cache store 같은 post-provider 단계가 실패하면 `AbstractAgentModelError`의 usage/routing receipt를 runner가 먼저 반영합니다. Durable run은 이미 발생한 model usage·cost·counter와 terminal MODEL evidence를 저장하고 checkpoint를 갱신하므로, resume이 소진된 token/cost/step budget을 초기화하거나 이미 청구된 model call을 재실행하지 않습니다.

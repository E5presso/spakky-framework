...
Spakky의 `ApplicationContext`는 같은 인터페이스의 복수 구현체가 존재할 때 단수 의존성에는 명시 선택 정책이 필요하며, **Decorator 자동 체이닝은 지원하지 않는다.** 이 ADR 작성 당시의 선택 순서는 이후 DI multi-implementation resolution 작업에서 `Qualifier/name` → application binding → `@Primary` → legacy parameter name → 단일 후보 순서로 정리되었다.

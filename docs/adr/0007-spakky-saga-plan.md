...
|---|---|
| UseCase 호출 (내부 서비스) | Repository 직접 접근 |
| Command 발행 (외부 서비스) | Aggregate 직접 조작 |
| 실패 시 보상 흐름 실행 | 비즈니스 규칙 판단 |
| step 간 데이터 전달 (`SagaData`) | 트랜잭션 관리 (`@Transactional`은 UseCase가 담당) |
...
Saga 실패는 정상적인 비즈니스 결과이므로 예외를 발생시키지 않는다.

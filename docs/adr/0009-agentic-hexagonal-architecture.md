...
Tool invocation의 기본 HITL primitive는 `ToolRisk`와 `ToolApprovalRequirement`에서 파생된다. Low-risk 또는 `NOT_REQUIRED` tool은 approval 없이 진행하고, write/side-effect/network/destructive boundary만 `AgentApprovalRequest`와 `AgentYield(kind=APPROVAL)`로 caller에게 노출한다. Approval decision signal은 `APPROVE`, `REJECT`, `MODIFY`, `DEFER`, `CANCEL`을 typed outcome으로 해석하며, `DEFER`는 `INTERRUPTED/WAITING_APPROVAL`, `REJECT`는 `FAILED`, `CANCEL`은 `CANCELLING`으로 분리해 waiting approval과 cancellation/failure lifecycle을 섞지 않는다.

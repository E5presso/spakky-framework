...
- 배치 `flush`가 실패하면 어느 메시지의 잘못인지 그 자리에서는 가릴 수 없으므로, 릴레이가 **같은 배치를 메시지 1건씩 다시 `send` + `flush`** 하여 브로커의 판정을 메시지에 귀속시킵니다. 거부된 메시지만 retry count를 올리고 자기 파티션 키를 보류하며, 나머지는 정상 발행됩니다. 이미 배치에서 전달된 레코드가 재확인 과정에서 다시 전달될 수 있으므로 소비자 멱등성이 전제입니다(at-least-once).
- 재확인은 실패 경로에서만 일어납니다. 정상 경로는 종전대로 배치 1개당 `flush` 1회이므로 처리량이 바뀌지 않습니다.
- 브로커가 오래 응답하지 않으면 배치의 모든 메시지가 재확인에서 실패하여 각자 retry count를 올립니다. 다만 실패한 메시지는 `claimed_at`이 남아 `claim_timeout_seconds`가 지나야 다시 조회되므로, 재시도 예산 소진에는 대략 `max_retry_count × claim_timeout_seconds`(기본값 기준 약 25분)의 지속 장애가 필요합니다. 그보다 짧은 장애는 예산을 다 쓰지 않습니다.

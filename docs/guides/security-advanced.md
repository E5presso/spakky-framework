...
| Cryptography | Snapshot sign/verify, hash, HMAC, password verify 성공 시 provider-native success 값 반환 | invalid signature, malformed envelope, expired snapshot, password verification failure는 auth port 노출 지점에서 `CHALLENGE / INVALID_CREDENTIAL` 반환 | signer/verifier/password provider 비가용은 `ERROR / VERIFICATION_PROVIDER_UNAVAILABLE` | `plugins/spakky-cryptography` auth provider test |

...
`spakky-cache` 플러그인을 로드하면 `CacheAspect`, `AsyncCacheAspect`가 등록됩니다. 실제 backend는 `spakky-redis`가 `RedisCache` Pod로 제공합니다. 이후 서비스 메서드에 `@cacheable()` 또는 `@cache_evict()`를 붙이면 됩니다.
...
`spakky-redis`는 Redis에 직렬화된 값을 저장하는 `spakky-cache` 백엔드인 `RedisCache[T]`를 제공합니다. 비즈니스 코드는 `ICache[T]`에 의존하면 되므로 cache hit/miss 처리 방식을 backend에 묶지 않습니다.

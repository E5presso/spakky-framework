...
- 동기·비동기 `send()`는 producer가 즉시 보고한 레코드 크기 거부만 `EventDeliveryRejectedError`로 올립니다. `flush()`도 동기 delivery callback의 `MSG_SIZE_TOO_LARGE`와 비동기 delivery future의 `MessageSizeTooLargeError`만 같은 공통 예외로 변환합니다. 동기 경로의 로컬 producer 큐 포화(`BufferError`)·그 밖의 `KafkaException`, 비동기 connection·timeout 오류를 포함한 나머지 실패는 원래 client 예외 타입을 보존하며, 한 배치에 두 종류가 섞이면 transport 장애를 우선합니다. 이 구분으로 영구적이고 특정 레코드에 귀속되는 거부만 호출자의 retry 예산을 소모합니다.

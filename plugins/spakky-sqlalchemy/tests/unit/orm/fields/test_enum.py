...
def test_enum_with_enum_class_expect_correct_class() -> None:
    ...
    field = EnumField(enum_class=SampleRole)
...
def test_enum_native_enum_false_expect_false() -> None:
    ...
    field = EnumField(enum_class=SampleRole, native_enum=False)
...
def test_enum_get_from_annotated_expect_instance() -> None:
    ...
    annotated = cast(
        AnnotatedType, Annotated[SampleRole, EnumField(enum_class=SampleRole)]
    )
    field = EnumField.get(annotated)
...
def test_enum_exists_in_annotated_expect_true() -> None:
    ...
    annotated = cast(
        AnnotatedType, Annotated[SampleStatus, EnumField(enum_class=SampleStatus)]
    )
    assert EnumField.exists(annotated) is True
...
def test_enum_with_different_enum_classes_expect_distinct() -> None:
    ...
    field_role = EnumField(enum_class=SampleRole)
    field_status = EnumField(enum_class=SampleStatus)

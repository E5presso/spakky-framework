...
from spakky.plugins.cryptography.cryptography.interface import ICryptor
from spakky.plugins.cryptography.error import DecryptionFailedError, KeySizeError
from spakky.plugins.cryptography.key import Key


def test_aes() -> None:
    """AES 암호화기로 암호화 후 복호화하면 원본 평문이 복원되고, 위변조된 데이터는 복호화 실패하는지 검증한다."""
...
def test_aes_expect_key_size_error() -> None:
    """지원되지 않는 키 크기로 AES를 생성하면 KeySizeError가 발생하는지 검증한다."""

...
MCP로 전송할 마크다운, 그리고 사용자에게 보여주는 마크다운 모두 해당:

**코드 블록/인라인 코드:**
- 코드 블록 열기/닫기 백틱(` ``` `)이 다른 마크다운 요소와 겹치지 않도록 확인
- 코드 블록 안에 백틱 3개가 포함되면 외부를 백틱 4개(` ```` `)로 감싸기
- 인라인 코드 안에 백틱이 포함되면 이중 백틱(` `` `)으로 감싸기

**특수 기호 이스케이프:**
- GitHub 이슈/PR body에서 `<`, `>` 는 HTML 태그로 해석됨 → `&lt;`, `&gt;` 또는 인라인 코드로 감싸기
- 테이블 셀 안의 `|` 는 `\|` 또는 인라인 코드로 감싸기
- `*`, `**`, `~`, `~~` 등 서식 기호가 의도치 않게 열리지 않도록 주의
  - 닫히지 않는 `*`나 `**`는 이후 텍스트 전체를 이탤릭/볼드로 오염시킴
  - 짝이 안 맞으면 이스케이프(`\*`) 처리

**구조 안정성:**
- 마크다운 출력 전 **열린 블록이 모두 닫혔는지** 확인 (코드 블록, 테이블, HTML 태그)
- 중첩 리스트는 들여쓰기를 정확히 맞추기 (2칸 또는 4칸 일관 유지)

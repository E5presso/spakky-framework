...
- **`@override` 필수**: 부모 메서드를 재정의할 때 `@override` 데코레이터가 누락되면 지적합니다.
- **`assert` 문 금지**: `src/` 내에서 `assert` 문 사용 시 지적합니다. 커스텀 에러로 처리합니다.
- **동적 속성 접근 금지**: `getattr()`, `hasattr()`, `setattr()` 사유 없이 사용 시 지적합니다.
...
- **Flaky 테스트 금지**: `time.sleep`, `datetime.now()` 직접 의존, 실행 순서 의존, 외부 네트워크 호출이 있으면 지적합니다.

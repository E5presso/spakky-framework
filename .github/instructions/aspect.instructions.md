...
|-----------------|------------|------|
| `@Aspect()` / `@AsyncAspect()` | `spakky.core.aop.aspect` | 동기/비동기 Aspect 등록 |
| `IAspect` / `IAsyncAspect` | `spakky.core.aop.interfaces.aspect` | Aspect 인터페이스 |
| `@Before` / `@After` / `@Around` / `@AfterReturning` / `@AfterRaising` | `spakky.core.aop.pointcut` | 포인트컷 |
| `@Order(n)` | `spakky.core.pod.annotations.order` | 실행 순서 (낮을수록 먼저) |
...
포인트컷 조건: `iscoroutinefunction(x)`로 동기/비동기 분리.
